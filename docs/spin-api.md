...
/init gameState now provides the authoritative startup grid through Line1.Slot11–Slot15, Line2.Slot21–Slot25 and Line3.Slot31–Slot35. LinesKoff maps Koff1–Koff10. Raw state is retained. CountFreeSpin counts played free spins, not remaining spins. It can return 0 on the first normal spin after a completed series; do not enforce monotonicity across completed series. CardSum is the total bet; SpinResult.LineSum is the per-line stake and SpinResult.Lines is the selected line count. These do not represent unpaid winnings. SumPay is the unpaid win amount and restores the collectible win when PayDate is empty; paid cards do not restore a collectible win. Pending-operation reconciliation still needs a backend contract; a last-card snapshot alone does not settle a pending request.
...
Completed Free Spins counter handling: after remaining reaches zero, the series snapshot is frozen until a new confirmed award starts a new series. A later valid /freespins count (including 0) cannot reopen that series or block normal spins. The prior winnings and payment ledger are retained. Decreasing counts during an active series remain invalid; a new award establishes a fresh baseline.
