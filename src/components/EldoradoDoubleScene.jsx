...
export default function EldoradoDoubleScene({
  ...
  return (
    <section className="eldorado-double doubling-desktop" aria-busy={loading}>
      <img
        className="eldorado-double__landscape"
        src={asset("sprite_001_1336x542_at_1_1.png")}
        alt=""
        aria-hidden="true"
      />
      <img
        className="eldorado-double__arches"
      ...
      <div className="double_content">
          ...
          <div className="doubling-desktop__levels">
            {levels.map((level, index) => (
              <div
                key={index}
                className={
                  "doubling-desktop__level" +
                  (index >= levels.length - 1 - step ? " --active" : "")
                }
              >
                <img src={asset(index >= levels.length - 1 - step ? "sprite_020_160x58_at_163_1826.png" : "sprite_011_160x59_at_211_1491.png")} alt="" />
                <span>{level}</span>
              </div>
            ))}
          </div>
        ...
        {lastPick && (
          <img
            className={"eldorado-double__choice-highlight --" + lastPick}
        ...
        <div className="eldorado-double__choices">
          {["left", "right"].map((side) => (
            <div
              key={side}
              className={
                "eldorado-double__choice --" +
                side +
                (lastPick === side ? " --" + lastStatus : "")
              }
            >
              <div className="eldorado-double__chest-frame">
  <img src={chestSource(side)} alt="" aria-hidden="true" />
        ...
        <img
          className="eldorado-double__character"
