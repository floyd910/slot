...
test('View 1 starts empty but restores symbols for an unpaid winning spin in every game',async()=>{
 ...
 try {
  ...
  for(const currentGame of ['ganchina-sokrovishch','marvorid-djemchug','khiradmandi-makor','egypt','kadima-drevnii','khocha-afandi','babylon','fruits']) {
   for(const hasRecoveredGrid of [false,true]) {
    ...
    const state={currentGame,grid,hasRecoveredGrid,hasSessionSpin:false,spinResult:{idCard:'unpaid-card',WinSum:100,creditedToBalance:false}};
    const render=state=>{
    ...
    if(hasRecoveredGrid)assert.ok(values.some(value=>value!==''),currentGame+' must restore the unpaid spin symbols');
    else assert.ok(values.every(value=>value===''),currentGame+' must have no old symbols');
